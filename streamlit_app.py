...
st.markdown("""
<style>
/* Hide Streamlit GitHub icon in header */
header [href*="github.com"] { display: none !important; }
...
def safe_group(players):
    """Beginner and Intermediate cannot exist together on the same court"""
...
# ======================================================
def take_four_safe():
    """Take first safe combination of 4 players from queue"""
...
with st.sidebar:
    ...
    st.download_button("📥 Download CSV", data=create_csv(),
                       file_name="pickleball_results.csv", mime="text/csv")
...
for cid in st.session_state.courts:
    ...
    scoreA = st.number_input("Score A", 0, key=f"A{cid}", step=1)
    scoreB = st.number_input("Score B", 0, key=f"B{cid}", step=1)
